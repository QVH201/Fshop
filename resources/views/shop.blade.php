          ...
          <div class="accordion-item mb-4">
            ...
            <div id="accordion-filter-price" class="accordion-collapse collapse show border-0"
              ...
              <input class="price-range-slider" type="text" name="price_range" value="" data-slider-min="0"
                data-slider-max="1000" data-slider-step="5" data-slider-value="[{{$from}},{{$to}}]" data-currency="$" />
              <div class="price-range__info d-flex align-items-center mt-2">
                <div class="me-auto">
                  <span class="text-secondary">Min Price: </span>
                  <span class="price-range__min">${{$from}}</span>
                </div>
                <div>
                  <span class="text-secondary">Max Price: </span>
                  <span class="price-range__max">${{$to}}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="d-flex justify-content-between mb-4 pb-md-2">
          ...
          <div class="shop-acs d-flex align-items-center justify-content-between justify-content-md-end flex-grow-1">
            <select class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0" aria-label="Sort Items"
              name="size" id="pagesize" form="frmfilter">
              <option value="12" @if($size==12) selected="selected" @endif>Show 12</option>
              <option value="24" @if($size==24) selected="selected" @endif>Show 24</option>
              <option value="48" @if($size==48) selected="selected" @endif>Show 48</option>
              <option value="102" @if($size==102) selected="selected" @endif>Show 102</option>
            </select>
            <select class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0" aria-label="Sort Items"
              name="order" id="orderby" form="frmfilter">
              <option value="-1" @if($order==-1) selected="selected" @endif>Default Sorting</option>
              <option value="1" @if($order==1) selected="selected" @endif>Date, new to old</option>
              <option value="2" @if($order==2) selected="selected" @endif>Date, old to new</option>
              <option value="3" @if($order==3) selected="selected" @endif>Price, low to high</option>
              <option value="4" @if($order==4) selected="selected" @endif>Price, high to low</option>
            </select>
...
<form id="frmfilter" method="GET" action="{{ route('shop.index') }}">
  ...
  <input type="hidden" name="categories" id="categories" value="{{$q_categories}}" />
  <input type="hidden" name="prange" id="prange" value="" />
  <!-- Add this to persist search query when filtering -->
  @if(request()->has('query'))
  <input type="hidden" name="query" value="{{request()->query('query')}}" />
  @endif
  @if(request()->has('search-keyword'))
  <input type="hidden" name="search-keyword" value="{{request()->query('search-keyword')}}" />
  @endif
</form>
...
<script>
    $(function(){
        ...
        // Assuming the slider library provides a way to bind to the change event.
        // Based on theme.js for 'price-range-slider', usually input event work or 'slideStop'.
        // For Bootstrap Slider:
        // $("input.price-range-slider").slider().on('slideStop', function(ev){ ... });
        // But since we can't be sure of the library version, we check the HTML structure.
        // The theme likely initializes it. We might need to hook into it.
        
        // Let's try to assume we can listen to the change or 'slideStop' event on the input
        $("input[name='price_range']").on("slideStop", function(ev){
           var val = $(this).val();
           $("#prange").val(val);
           $("#frmfilter").submit();
        });
